...
* Wedding Video Booth â€” Cloudflare Pages Function
*
* FIXES v2:
...
* 3. VN di galeri pakai vn_drive_id â€” frontend fetch via /api/vn-proxy?id=DRIVE_ID
* 4. Hapus driveViewUrl untuk VN â€” pakai driveAudioUrl yang benar
* 5. Export semua named handlers + generic fallback
*
* Google Sheet tab "Entries" â€” baris 1 header:
* A:timestamp B:name C:message D:media_url E:vn_url F:drive_id G:vn_drive_id
H:media_type
*
...
async function dispatch({ request, env, next }) {
 ...
 if (pathname === '/api/submit' && method === 'POST') return await
handleSubmit(request, env);
 ...
 if (pathname === '/api/vn-proxy' && method === 'GET') return await
handleVNProxy(request, env);
...
async function handleSubmit(request, env) {
 ...
 const folderId = await driveCreateFolder(token, env.GOOGLE_DRIVE_FOLDER_ID,
`${stamp}_${safeName}`);
 // Upload media â€” resumable agar tidak timeout
 const mediaDriveId = await driveResumableUpload(token, folderId, mediaName, mediaMime,
mediaFile);
 ...
 vnDriveId = await driveResumableUpload(token, folderId, 'voicenote.webm', vnMime,
vnFile);
 ...
 video_url: mediaUrl,
 photo_url: mediaUrl,
 vn_url: vnUrl, // /api/vn-proxy?id=... â€” bisa langsung diplay
...
async function handleGallery(env) {
 ...
 // - Jika kolom E sudah berisi /api/vn-proxy?id=... â†’ pakai langsung
 // - Jika kolom E berisi URL Drive lama (thumbnail/drive.google.com) â†’ convert ke
proxy
 // - Jika kosong tapi kolom G ada drive_id â†’ buat proxy URL baru
 let vnUrl = r[4] || '';
 if (vnDriveId) {
 // Selalu pakai vn_drive_id untuk construct proxy URL yang benar
 vnUrl = `/api/vn-proxy?id=${vnDriveId}`;
 } else if (vnUrl && !vnUrl.startsWith('/api/vn-proxy')) {
 // VN URL lama dari Drive langsung â€” clear karena tidak bisa diplay
 vnUrl = '';
 }
 return {
 timestamp: r[0] || '',
...
async function handleVNProxy(request, env) {
 ...
 // alt=media â†’ streaming file content langsung
...
async function handleDebug(env) {
 ...
 r.sheet_header = h.values?.[0] ?? '(kosong â€” cek nama tab)';
...
async function googleToken(env) {
 if (!env.GOOGLE_SERVICE_ACCOUNT_EMAIL) throw new Error('GOOGLE_SERVICE_ACCOUNT_EMAIL tidak
di-set');
...
https://www.googleapis.com/auth/spreadsheets';
 ...
 const cla = b64u(JSON.stringify({ iss: email, scope, aud:
...
async function importRsa(pem) {
 ...
 return crypto.subtle.importKey('pkcs8', buf.buffer, { name: 'RSASSA-PKCS1-v1_5', hash:
'SHA-256' }, false, ['sign']);
}
//
â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
async function sheetsAppend(token, sheetId, values) {
 const url =
`https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/Entries!A:H:append?
...
uploadType=resumable&fields=id&supportsAllDrives=true',
 ...
 if (!uploadRes.ok) throw new Error(`Drive upload ${uploadRes.status}: ${uploadTxt.slice(0,
200)}`);
 const { id } = JSON.parse(uploadTxt);
 // Step 3: Set public permission (untuk media/foto â€” VN tidak perlu public karena via
proxy)
...
function b64u(s) { return btoa(unescape(encodeURIComponent(s))).replace(/\+/g,'-
').replace(/\//g,'_').replace(/=/g,''); }
function b64uBuf(b) { return btoa(String.fromCharCode(...new
Uint8Array(b))).replace(/\+/g,'-').replace(/\//g,'_').replace(/=/g,''); }
